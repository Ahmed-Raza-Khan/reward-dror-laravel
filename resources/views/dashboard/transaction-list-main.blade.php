@extends('layouts.app')

@section('title', 'Reward Dror - Transactions List')

@section('content')
    <main id="main-content" class="flex-1 p-8 transition-all duration-300 lg:ml-64 business-api-main-heading">
        <div class="max-w-full mx-auto bg-white p-6 rounded-xl shadow-lg border border-gray-100">
            <div class="flex justify-between items-center mb-8">
                <h1 class="text-[35px] font-medium text-[#333333]">Transactions</h1>
                <button class="text-white text-[16px] px-6 py-3 rounded-full font-semibold transition-colors system-settings-background-button">
                    Export (CSV, PDF)
                </button>
            </div>

            <div class="flex justify-between items-center mb-6">
                <div class="flex items-center gap-3">
                    <span class="text-[#000000] text-[16px] font-regular">Show</span>
                    <div class="relative">
                        <select class="appearance-none border border-[#FF7F32] rounded-md px-4 py-1 pr-5 text-[14px] font-regular text-[#000000] focus:outline-none">
                            <option>5</option>
                            <option>10</option>
                        </select>
                        <div class="absolute right-2 top-6 -translate-y-1/2 pointer-events-none">
                            <svg class="w-4 h-4 text-[#000000]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                        </div>
                    </div>
                </div>

                <div class="flex items-center gap-3">
                    <div class="relative">
                        <input type="text" id="businessSearch" placeholder="Search" class="pl-4 pr-10 py-2 border border-gray-200 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-[#FF7F32] w-54">
                        <span class="absolute right-3 top-2.5 text-[#000000]">
                            <img src="/assets/images/search-icon-nav-dark.svg" alt="search">
                        </span>
                    </div>
                    <button class="main-button-background-fee-config p-3 rounded-md text-white">
                        <img src="/assets/images/search-filter-dashed-management.png" alt="filtre" class="w-7 h-5">
                    </button>
                </div>
            </div>

            <div class="overflow-x-auto">
                <table class="w-full text-left border-separate border-spacing-y-3">
                    <thead>
                        <tr class="text-[#000000] text-[15px] capitalized tracking-wider">
                            <th class="px-4 py-2 font-medium">S.No</th>
                            <th class="px-4 py-2 font-medium">Transaction ID</th>
                            <th class="px-4 py-2 font-medium">Payment Method</th>
                            <th class="px-4 py-2 font-medium">Amount (Base + Fee)</th>
                            <th class="px-4 py-2 font-medium">Date/Time</th>
                            <th class="px-4 py-2 font-medium text-center">Status</th>
                            <th class="px-4 py-2 font-medium">Provider Transaction ID</th>
                            <th class="px-4 py-2 font-medium text-center">Action</th>
                        </tr>
                    </thead>
                        <tbody id="transaction-body" class="fee-configration-main-tick-fee">
                    </tbody>
                </table>
            </div>

            <div class="bg-white p-1 flex items-center justify-between mt-3">
                <p class="text-[#000000] text-[16px] font-regular">Showing 1 to 10 from 60 entries</p>
                <div class="flex items-center border border-gray-100">
                    <button class="px-5 py-3 rounded text-[16px] text-medium pagination-hover-background hover:text-[#FFFFFF]">Previous</button>
                        <a href="/transaction-list-main"><button class="px-5 py-3 rounded text-[17px] font-regular text-white pagination-hover-background-null">1</button></a>
                            <span class="px-2 text-[#000000]">|</span>
                        <a href="/transaction-list-main"><button class="px-5 py-3 rounded text-[17px] font-regular text-[#000000] hover:text-[#FFFFFF] pagination-hover-background">2</button></a>
                            <span class="px-2 text-[#000000]">|</span>
                        <a href="/transaction-list-main"><button class="px-5 py-3 rounded text-[17px] font-regular text-[#000000] hover:text-[#FFFFFF] pagination-hover-background">3</button></a>
                            <span class="px-2 text-[#000000]">|</span>
                    <button class="px-5 py-3 rounded text-[16px] text-medium pagination-hover-background hover:text-[#FFFFFF]">Next</button>
                </div>
            </div>
        </div>
    </main>

    <script src="assets/js/dashboard/transaction-list.js"></script>
@endsection